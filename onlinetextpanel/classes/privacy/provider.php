...
namespace assignsubmission_onlinetextpanel\privacy;
...
class provider implements
    ...
    public static function get_metadata(collection $collection): collection {
        $detail = [
                    ...
                    'onlinetextpanel' => 'privacy:metadata:textpurpose'
                  ];
        $collection->add_database_table('assignsubmission_onlinetextpanel', $detail, 'privacy:metadata:tablepurpose');
    ...
    public static function export_submission_user_data(assign_plugin_request_data $exportdata) {
        ...
        $plugin = $assign->get_plugin_by_type('assignsubmission', 'onlinetextpanel');
        $submission = $exportdata->get_pluginobject();
        $editortext = $plugin->get_editor_text('onlinetextpanel', $submission->id);
        $context = $exportdata->get_context();
        if (!empty($editortext)) {
            $submissiontext = new \stdClass();
            $currentpath = $exportdata->get_subcontext();
            $currentpath[] = get_string('privacy:path', 'assignsubmission_onlinetextpanel');
            $submissiontext->text = writer::with_context($context)->rewrite_pluginfile_urls($currentpath,
                    'assignsubmission_onlinetextpanel', 'submissions_onlinetextpanel', $submission->id, $editortext);
            writer::with_context($context)
                    ->export_area_files($currentpath, 'assignsubmission_onlinetextpanel', 'submissions_onlinetextpanel', $submission->id)
    ...
    public static function delete_submission_for_context(assign_plugin_request_data $requestdata) {
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($requestdata->get_context()->id, 'assignsubmission_onlinetextpanel',
                ASSIGNSUBMISSION_onlinetextpanel_FILEAREA);

        // Delete the records in the table.
        $DB->delete_records('assignsubmission_onlinetextpanel', ['assignment' => $requestdata->get_assignid()]);
    }
    ...
    public static function delete_submission_for_userid(assign_plugin_request_data $deletedata) {
        ...
        // Delete related files.
        $fs = get_file_storage();
        $fs->delete_area_files($deletedata->get_context()->id, 'assignsubmission_onlinetextpanel', ASSIGNSUBMISSION_onlinetextpanel_FILEAREA,
                $submissionid);

        // Delete the records in the table.
        $DB->delete_records('assignsubmission_onlinetextpanel', ['assignment' => $deletedata->get_assignid(),
    ...
    public static function delete_submissions(assign_plugin_request_data $deletedata) {
        ...
        $fs->delete_area_files_select($deletedata->get_context()->id,
                'assignsubmission_onlinetextpanel', ASSIGNSUBMISSION_onlinetextpanel_FILEAREA, $sql, $params);

        $params['assignid'] = $deletedata->get_assignid();
        $DB->delete_records_select('assignsubmission_onlinetextpanel', "assignment = :assignid AND submission $sql", $params);
    }
}
